...
class ClientService extends BaseService
{
    ...
    {
        $status = Status::findByNameAndType('status_active', 'Client')->id;
    ...
    public function update()
    {
        ...
        $this->when($this->model->isDirty(), function (ClientService $service) {
            $service->notify('Client_updated');
        });

        $this->model->save();

        $this->when(request()->get('roles'), function (ClientService $service) {
    ...
    {
        if ($Client->isAppAdmin() && !$Client->isInvited())
